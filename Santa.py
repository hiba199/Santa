import random
from lithops.multiprocessing import Process, Semaphore
import time
...
def main():
    thread = Process(target=santa)  # main thread for SantaClaus
    thread.start()  # starting the thread
    for i in range(9):
        reindThread.append(Process(target=reindeer))
    for j in range(9):
        elfThread.append(Process(target=elves))
    for t in elfThread:
        t.start()
